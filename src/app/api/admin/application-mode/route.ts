...
const parseMode = (value: unknown): ApplicationMode | null =>
  value === "DEMO" || value === "PRODUCTION" ? value : null;
...
export async function PUT(request: Request) {
  ...
  if (!mode)
    return NextResponse.json(
      { error: "Choose Demo or Production mode." },
      { status: 400 },
    );
  ...
  if (mode !== before) {
    ...
    if (mode === "PRODUCTION")
      for (const session of identityRepository.read().sessions)
    ...
    recordAudit(
      actor,
      {
        event: "application.mode.updated",
        resourceType: "organization",
        resourceId: identity.organization.id,
        before: { mode: before },
        after: { mode },
      },
