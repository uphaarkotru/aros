export const revenueRoles=["SDR","AE","RSM","SALES_ENGINEER","SALES_ENGINEER_MANAGER","PARTNER_SALES","VP_SALES","CRO"] as const;
...
export type ParticipationType="OWNER"|"SDR_SUPPORT"|"SALES_ENGINEERING"|"PARTNER"|"EXECUTIVE_SPONSOR"|"OVERLAY"|"CUSTOM";
...
export interface Organization {id:string;name:string;slug:string;primaryDomain:string|null;status:OrganizationStatus;environment:OrganizationEnvironment;timezone:string;fiscalYearStartMonth:number|null;defaultMethodology:Methodology|null;createdAt:string;updatedAt:string}
export interface User {id:string;email:string;firstName:string;lastName:string;displayName:string;avatarUrl:string|null;status:UserStatus;platformRole:PlatformRole|null;lastLoginAt:string|null;createdAt:string;updatedAt:string;passwordHash:string;isDemoUser:boolean;
 ...
 organizationId:string;role:RevenueRole;managerUserId:string|null;teamId:string|null;regionId:string|null;timezone:string|null;isAdmin:boolean}
...
export interface OrganizationRoleDefinition {id:string;organizationId:string;name:string;code:string;systemTemplateId:string|null;category:RoleCategory;description:string|null;defaultExperienceKey:string|null;permissions:string[];isSystemSeeded:boolean;isActive:boolean;createdAt:string;updatedAt:string}
