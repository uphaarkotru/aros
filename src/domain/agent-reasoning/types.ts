...
export interface AgentReasoningMetadata{reasonerId:string;reasonerVersion:string;simulated:true;provider?:string;model?:string;promptVersion?:string;contextVersion:string;outputSchemaVersion:string;requestId:string;traceId:string;startedAt:string;completedAt:string;latencyMs:number;retryCount:number;rawOutputFormat:AgentReasonerResponseFormat;parseStatus:"passed"|"failed";tokenUsage?:number;costEstimate?:number;}
