...
export type LLMProviderErrorCode="timeout"|"rate-limit"|"provider-unavailable"|"transient-failure"|"invalid-json"|"schema-violation"|"malformed-output"|"invalid-request"|"unsupported-model"|"replay-missing"|"retry-exhausted";
...
export interface LLMProvider{id:string;name:string;version:string;generate(request:LLMRequest):Promise<LLMResponse>;validate(request:LLMRequest):Promise<LLMValidationResult>;healthCheck():Promise<LLMHealthCheckResult>;}
...
export type ModelStatus="active"|"experimental"|"deprecated"|"unavailable";
